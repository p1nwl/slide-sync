  ...
  createPresentation: async (req: Request, res: Response) => {
    try {
      ...
      if (!title || !creatorNickname || !creatorUserId) {
        console.warn(
          "[CONTROLLER] Create presentation request missing required fields:",
          { title, creatorNickname, creatorUserId: !!creatorUserId }
        );
      ...
      const newPresentation: IPresentation = new Presentation({
        title,
        slides: [{ elements: [], order: 0 }],
        users: [
          { id: creatorUserId, nickname: creatorNickname, role: "creator" },
        ],
      });
